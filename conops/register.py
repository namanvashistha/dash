...
def list_apps():
    """The list endpoint has returned both a bare array and a {"apps": [...]}
    envelope across versions, so handle either rather than assuming."""
    data = request("GET", "/api/v1/apps/")
...
def main():
    ...
    try:
        existing = {a.get("name") for a in list_apps()}
    ...
    print("\nRegistered apps:")
    for a in sorted(list_apps(), key=lambda x: x.get("name") or ""):
        print(
            f"  {a.get('name', '?'):12s} "
            f"{a.get('status', '?'):10s} "
            f"{a.get('branch', '?')}"
